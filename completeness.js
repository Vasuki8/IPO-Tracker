...
function renderQualityDashboard(audit, queue) {
  ...
  root.innerHTML = `<section class="quality-panel">
    ...
    <div class="quality-body">
      ...
      <div class="quality-subcard"><h3>Next IPOs to update</h3><p>Up to eight records, ordered by IPO stage, recency and missing official details.</p><div class="quality-queue-list">${queueRows.length ? queueRows.map((row) => `<div class="quality-queue-row"><div><a href="${escapeAttr(row.profilePath || `ipo/${row.id}/`)}"><strong>${escapeHtml(row.company || row.id || 'IPO')}</strong></a><span>${row.openDate ? `${row.openDate < today ? 'Opened' : 'Opens'} ${prettyDate(row.openDate)}` : escapeHtml(row.stage || '')}</span></div><span class="quality-priority">${escapeHtml(row.priorityLabel || '')}</span><span class="quality-missing">${Number(row.missingFieldCount || 0)} gaps · ${qualityPct(row.completenessPct)}${row.sourceReviewCount ? ` · ${Number(row.sourceReviewCount)} source reviews` : ''}</span></div>`).join('') : '<div class="quality-empty">No records are awaiting an update in this report.</div>'}</div></div>
    </div>
  </section>`;
}
