...
class ReviewedFinalNoticeTests(unittest.TestCase):
    ...
    def test_final_prospectus_fill_preserves_evidence_and_is_idempotent(self):
    ...
    def test_non_final_notice_cannot_fill_static_field(self):
        entry = copy.deepcopy(self.entry)
        entry['source'] = {
            'name': 'Issuer final notice',
            'url': 'https://example.com/final-notice.pdf',
            'kind': 'offer-notice',
        }
        applied, conflicts = fill_reviewed_fields(self.rows, [entry])
        self.assertEqual(applied, 0)
        self.assertTrue(conflicts)
        self.assertNotIn('lotSize', self.rows['example'])
        self.assertIn('Final Prospectus', conflicts[0]['reason'])
    ...
    def test_final_prospectus_fill_is_independent_of_older_migration_conflicts(self):
            ...
            'changes': [
                {
                    'id': 'example',
                    'field': 'registrar',
                    'beforeHash': fingerprint(None),
                    'after': 'Earlier Services Limited',
                }
            ],
        ...
        entries = [
            {
                **self.entry,
                'id': 'example',
                'field': field,
                'beforeHash': fingerprint(row[field]),
                'after': after,
            }
            for field, after in [
                ('exchange', 'BSE'),
                ('issueComposition', {'freshShares': 0, 'ofsShares': 100}),
            ]
        ]
