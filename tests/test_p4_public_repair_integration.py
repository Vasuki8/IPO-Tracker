...
class P4PublicRepairIntegrationTests(unittest.TestCase):
    ...
    def test_teamtech_same_document_stays_held_without_erasing_its_review(self):
        ...
        # The complete source review found contradictory units on page 89.
        # A structurally valid extraction from the same PDF cannot resolve it.
        ...
        self.assertNotIn('objectsOfIssue', view)
        decision = view['publicQuality']['fields']['objectsOfIssue']
        self.assertEqual(decision['state'], 'under_review')
        self.assertEqual(snapshot['sourceEvidence']['sha256'],
                         ENTRIES['teamtech']['evidence']['sha256'])
        self.assertIn(ENTRIES['teamtech']['source']['url'],
                      [doc['url'] for doc in view['documents']])
