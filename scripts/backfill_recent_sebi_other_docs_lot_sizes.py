...
def backfill(payload: dict[str, Any], session: requests.Session, max_listing_pages: int = 40, limit: int = 125, max_pdfs: int = 5, pause: float = 0.12) -> dict[str, Any]:
    ...
    records.sort(key=lambda r: (r.get('sebiOtherDocumentsAttempt') or {}).get('checkedAt', ''))
    if limit > 0:
        records = records[:limit]

    pages, errors = discover_pages(session, records, max_listing_pages, pause)
    for record in records:
        record['sebiOtherDocumentsAttempt'] = {'checkedAt': core.now_ist().isoformat(timespec='seconds'), 'status': 'discovery_failed' if errors else 'no_matching_official_document', 'sourceUrl': SEBI_OTHER_DOCS_URL}
    ...
    for page in pages:
        ...
        try:
            ...
            record['sebiOtherDocumentsAttempt'].update(status='no_usable_lot_size', sourcePage=page.url)
            discovered_docs += len(docs)
        except Exception as exc:
            record['sebiOtherDocumentsAttempt'].update(status='source_blocked', error=str(exc)[:250])
            failed += 1
        ...
        for doc in docs[:max_pdfs]:
            ...
            try:
                ...
                if apply_lot(record, lot, doc, pages_read, page_count):
                    record['sebiOtherDocumentsAttempt'].update(status='updated', documentUrl=doc.get('url'))
                    updated += 1
