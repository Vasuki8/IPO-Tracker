...
const review={schema_version:"1.0.0",status:"reviewed_bse_issue_summary_high_priority_reconciliation",actions:Array.from({length:5},()=>({})),auto_import_allowed:false,
 sources:[
  source("a","https://www.sebi.gov.in/a.pdf"),source("b","https://nsearchives.nseindia.com/b.pdf"),source("c","https://www.bseindia.com/c.pdf"),
...
const rb=Buffer.from(JSON.stringify(review));assert.equal(validateReview(review).keys.size,9);
...
try{
 ...
 assert.equal(receipt.documents.length,9);assert.equal(fs.readdirSync(dir).length,9);assert.deepEqual(validateReceipt(receipt,review,rb),{documents:9,bytes:receipt.total_response_bytes});
...
console.log(JSON.stringify({bse_reconciliation_evidence_tests:{trusted_hosts:true,projection_hashes:true,documents:9,receipt:true}}));
