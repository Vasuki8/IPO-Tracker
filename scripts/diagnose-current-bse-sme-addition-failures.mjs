...
const targetIds = new Set(["20241211-15", "20241202-11", "20240722-21"]);
const failures = Object.values(state.notices || {})
  .filter((entry) => targetIds.has(entry?.notice_no) && entry?.status === "unparseable")
  .sort((a, b) => b.notice_no.localeCompare(a.notice_no));

if (failures.length !== targetIds.size) {
  throw new Error("expected exactly three retained cursor5 unparseable notices");
}
...
for (const entry of failures) {
  ...
  if (!response.ok) throw new Error("HTTP " + response.status + " " + entry.notice_no);
