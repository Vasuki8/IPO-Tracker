...
assert.deepEqual(missingHistoricalPdfFields(record), ["issue_price","issue_size_inr","minimum_bid_quantity"]);
assert.ok(candidateHistoricalPdf(record, 2026));
assert.equal(historicalPdfCandidates([{record}], {issuers:{}}, 2026, 10).length, 1);

const pages = [
  "Issue Price ₹125 per Equity Share. Total Issue Size ₹500 crore. Bid Lot 120 Equity Shares."
];
const result = applyHistoricalPdfFields(record, record.documents[0], pages, "2026-09-23T20:00:00Z");
assert.deepEqual(result.changed.sort(), ["issue_price","issue_size_inr","minimum_bid_quantity"]);
