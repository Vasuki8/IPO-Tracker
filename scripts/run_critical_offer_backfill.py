...
HTML filing landing page from ever being handed to pypdf. The canonical legacy
offer parser supplies separate deep financial/shareholding page budgets,
supplemental-notice rejection, final combined-ownership layouts and bounded
retries for truncated PDF transfers.
"""
...
import legacy_offer_parser as offer_parser  # noqa: E402

# Use the canonical legacy parser for full-document fallbacks too.
base.offer = offer_parser.base
...
def safe_choose_fallback_document(record, *, exclude_url=None):
    ...
    for doc in record.get("documents") or []:
        if not isinstance(doc, dict) or offer_parser.is_supplemental_document(doc):
            continue
