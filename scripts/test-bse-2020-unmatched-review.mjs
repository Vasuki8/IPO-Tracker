...
assert.deepEqual(validateReview(review),{actions:4,sources:8});
...
try{
  ...
  assert.equal(receipt.documents.length,8);assert.equal(fs.readdirSync(dir).length,8);
  assert.deepEqual(validateReceipt(receipt,review,reviewBytes),{documents:8,bytes:receipt.total_response_bytes});
...
console.log(JSON.stringify({bse_2020_unmatched_review_tests:{issuers:4,sources:8,recovery_collisions:0,materializer:true,publication_change:false}}));
