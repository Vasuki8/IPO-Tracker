...
const bseOnly = buildBseOnlyRecoveryRecord(
  {
    year: 2025,
    issuer_name: "3B Films Limited",
    kind: "listing_notice",
    inclusion: "ipo",
    url: "https://www.bseindia.com/markets/MarketInfo/DispNewNoticesCirculars.aspx?page=20250605-49",
    publication_date: "2025-06-05"
  },
  n,
  "2026-09-23T20:00:00Z"
);
assert.equal(bseOnly.id, "3b-films-limited");
assert.equal(bseOnly.board, "SME");
assert.equal(bseOnly.status, "listed");
assert.equal(bseOnly.listing_date.value, "2025-06-06");
assert.equal(bseOnly.issue_price.value, 50);
assert.equal(bseOnly.market_lot.value, 3000);
assert.equal(bseOnly.nse_symbol, null);
assert.equal(bseOnly.documents[0].type, "BSE Listing Notice");
assert.equal(bseOnly.status_evidence[0].document_type, "BSE Listing Notice");

assert.equal(
  buildBseOnlyRecoveryRecord(
    { year: 2025, issuer_name: "3B Films Limited", kind: "listing_notice", url: "https://www.bseindia.com/x" },
    n,
    "2026-09-23T20:00:00Z"
  ),
  null
...
assert.equal(
  buildBseOnlyRecoveryRecord(
    { year: 2025, issuer_name: "Wrong Limited", kind: "listing_notice", inclusion: "ipo", url: "https://www.bseindia.com/x" },
    n,
    "2026-09-23T20:00:00Z"
  ),
  null
