...
The retained draft source currently contains **90 companies / 91 DRHP/UDRHP source documents**. The lifecycle-aware reconciliation currently leaves **82 visible Pre-IPO companies** and hides **8 issuers that have already progressed** in the published IPO dataset. PR **#252** merged as `a5b797b047abffe6406559461bc79e0ace88664c`.

The latest eight-page scan contained **92 observations**, including **two duplicates**, for **90 unique filings observed in that scan**. One earlier retained filing remains in the union, producing **91 retained source documents across 90 companies**. SEBI page totals varied between 2,214 and 2,212, so the site explicitly labels incomplete coverage. Older years, unlabelled draft rows, exchange-only filings, addenda and corrigenda are outside this release's scope.
