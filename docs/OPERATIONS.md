...
`source_review_holds.py` supplies the same active hold bindings to public projection and validation. A restored canonical value does not close a document conflict: its manual source-review task and P4 blocker remain while issuer, offer and PDF identity still match. Resolve these tasks using the retained review and authoritative source evidence, including an explicit superseding disclosure when required. They do not create artificial missing fields or force another automatic parse. Existing value-scoped holds also remain actionable until their source/value binding changes.
