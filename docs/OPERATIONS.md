...
The final parser recognizes bidding ranges in definitions that place the minimum and maximum prices before parenthetic Floor Price and Cap Price labels. Both labelled values must belong to the same definition, and conflicting explicit ranges remain unresolved. This allows an evidenced bidding range to replace an older one-point band while keeping the final issue price separate.
