...
Release completed through PRs [#146](https://github.com/Vasuki8/IPO-Tracker/pull/146) and [#147](https://github.com/Vasuki8/IPO-Tracker/pull/147), accepted data commit `794a8f6012047a0d817f4363a9374439c9892eb3`. Both PRs passed all 1,283 Linux regressions. Pages run `35404502951`, live workflow `35404532544` and main validation `35404503868` succeeded. All 31 checked public files, including all eight additions, match their committed bytes. Live browser checks confirm issuer headings, official source links, unavailable terms and unknown source timestamps; the previously visited Vinod route needed a fresh browser load after deployment. The stable route’s HTTP bytes also match. See `release.json`, `live-delivery.json` and `browser-verification.json`.

**Exact next cohort:** reconcile the 12 BSE August 2026 unmatched SME candidates in `next-cohort.json` using the retained archive and issuer/symbol/date detail proofs. First resolve candidates already linked to a differently named tracker entry. Separately recover stable SEBI draft pagination and retry the smallest failed NSE historical cohort from an environment where the official endpoint responds. Reuse the verified RHP, final-offer, Other Documents and BSE historical responses; do not recrawl them. No P5/numerical or performance expansion is authorized by this audit. Source redistribution/commercial rights remain unresolved.
