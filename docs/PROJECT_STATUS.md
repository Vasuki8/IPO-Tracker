...
Validation on PR head `e0991a4db3502507eecd332a775b18e9341ae64a`: **1,404 uv/Python tests passed** in run **35461049702**, including the requested missing-observation, unchanged-observation/new-collection, failed/deferred/unavailable collection, delayed publication, stale secondary/current official, active provisional, unresolved proposal and timezone cases. The same run passed JavaScript syntax checks, Python/JavaScript operational contract fixtures, health-report reproduction, strict data validation, public payload generation and compact-support checks. The status-only follow-up head `921acba12d0fcd8ea32db70ca18541c7a3ee95fe` passed the same gate in run **35461191125**.

Release closeout is verified. PR #178 merged as **`84efd28f7431f8e714381ca912888b5aa3f4aea9`**. Post-merge validation **35461276708** passed; Pages **35461276152** deployed the merge successfully; immutable public-release verifier **35461302186** passed against that deployed commit. No canonical IPO values, P4 source repairs, P5/performance state or public behavior changed.
...
No P4 numerical repair, freshness tooling, source-review weakening or P5/performance expansion is part of this milestone. Release closeout is now verified. PR #175 merged as **`cf8849be375ba7850dbe0bb43c99760ef0589751`**. Final documented-head validation **35456730972** passed, post-merge validation **35456779950** passed, and GitHub Pages run **35456779156** deployed the exact merge commit successfully. Live public-release run **35456822588** then passed its immutable HTTPS verification against `https://vasuki8.github.io/IPO-Tracker/`, reporting **`expectedCommit: cf8849be...`**, **`status: passed`** and **`routeCount: 1436`**. The live verifier's stated scope is public-artifact/review delivery rather than independent source accuracy; source accuracy remains bounded by the retained official audit evidence above. The pre-merge browser run **35456596086** separately exercised all 30 newly admitted profiles and directory searches.
